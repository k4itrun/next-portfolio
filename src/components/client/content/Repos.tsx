...
export const Repos = () => {
  const { data: _repositories } = useSWR<IRepository[]>('/api/repos');
    ...
    <>
      <div className="mx-auto max-w-7xl my-24">
        <h1 className="font-display text-5xl font-medium sm:text-5xl text-gray-900 dark:text-white">
          My{' '}
          <span className="relative whitespace-nowrap text-color-layout">
            <svg className="absolute -mt-1 ml-1 top-3/5 left-0 h-[0.45em] w-full fill-color-layout rotate-180" viewBox="0 0 418 42" aria-hidden="true" preserveAspectRatio="none">
          ...
          <p className="text-xl text-gray-600 dark:text-white/50 font-normal">More than {repositories?.length || '1'} repositories that may be useless.</p>
        </h1>
              ...
              ? repositories
                  ...
                  .map((repo, index) => (
                    <a key={index} href={`https://github.com/${repo.full_name}`} target="_blank" rel="noreferrer" className="group relative bg-gray-100 dark:bg-gray-600/5 dark:shadow-2xl border border-gray-300 hover:border-color-layout dark:hover:border-color-layout dark:border-gray-500/25 hover:shadow-xl rounded-lg p-2.5 transition-all duration-200">
                      ...
                      <p className="text-md text-gray-600 dark:text-white/50">{repo.description?.length > 0 ? (repo.description.length > 40 ? `${repo.description.substring(0, 40)}...` : repo.description) : 'Did you know that this repository is unique?'}</p>
                      <div className="mt-5 flex justify-between items-center">
                        ...
                        <div className="flex items-center">
                          <img src={`https://skillicons.dev/icons?i=${repo.language?.replace(/\.| /g, '').toLowerCase()}`} alt={repo.language} className="w-4 h-4 mr-2" />
                          <p className="text-sm text-gray-900 dark:text-white bg-gray-200 dark:bg-neutral-700/25 px-2 py-1 rounded-md">{repo.language || 'Empty'}</p>
                        </div>
