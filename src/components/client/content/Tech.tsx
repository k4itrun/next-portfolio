import { tech } from "@9ll-fun/config";

export const Techs = () => {
 return (
  <>
   <div className="mx-auto max-w-7xl my-24">
    <h1 className="font-display text-5xl font-medium sm:text-5xl text-black dark:text-white text-right">
     My{" "}
     <span className="relative whitespace-nowrap text-color-layout">
      <svg className="absolute -mt-1 ml-1 top-3/5 left-0 h-[0.45em] w-full fill-color-layout rotate-180" viewBox="0 0 418 42" aria-hidden="true" preserveAspectRatio="none">
       <path d="M203.371.916c-26.013-2.078-76.686 1.963-124.73 9.946L67.3 12.749C35.421 18.062 18.2 21.766 6.004 25.934 1.244 27.561.828 27.778.874 28.61c.07 1.214.828 1.121 9.595-1.176 9.072-2.377 17.15-3.92 39.246-7.496C123.565 7.986 157.869 4.492 195.942 5.046c7.461.108 19.25 1.696 19.17 2.582-.107 1.183-7.874 4.31-25.75 10.366-21.992 7.45-35.43 12.534-36.701 13.884-2.173 2.308-.202 4.407 4.442 4.734 2.654.187 3.263.157 15.593-.78 35.401-2.686 57.944-3.488 88.365-3.143 46.327.526 75.721 2.23 130.788 7.584 19.787 1.924 20.814 1.98 24.557 1.332l.066-.011c1.201-.203 1.53-1.825.399-2.335-2.911-1.31-4.893-1.604-22.048-3.261-57.509-5.556-87.871-7.36-132.059-7.842-23.239-.254-33.617-.116-50.627.674-11.629.54-42.371 2.494-46.696 2.967-2.359.259 8.133-3.625 26.504-9.81 23.239-7.825 27.934-10.149 28.304-14.005.417-4.348-3.529-6-16.878-7.066Z" />
      </svg>
      <span className="relative">technologies</span>
     </span>
     <p className="text-xl text-black/50 dark:text-white/50 font-normal">I experiment with random technologies.</p>
    </h1>
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 mt-12">
     {tech
      ? tech
        ? tech.map((tech, index) => (
           <div key={index} className="group flex justify-between items-center relative bg-gray-600/5 dark:shadow-2xl overflow-hidden border border-gray-500/25 hover:border-color-layout hover:shadow-xl rounded-lg p-2.5 transition-all duration-200">
            <img src={tech.src} width="32" height="32" className="bg-white/5 p-1 w-[32px] h-[32px] rounded-md" alt={tech.name} />
            <div className="relative">
             <h2 className="font-display text-s font-medium text-black dark:text-white">{tech.name}</h2>
            </div>
           </div>
          ))
        : Array.from({ length: 20 }).map((_, index) => (
           <div key={index} className="group flex justify-between items-center relative bg-gray-200 dark:bg-neutral-700/20 overflow-hidden border-gray-500/25 rounded-lg p-2.5 transition-all duration-200">
            <div className="bg-neutral-300 dark:bg-neutral-700/50 animate-pulse p-1 w-[32px] h-[32px] rounded-md" />
            <div className="relative flex-1 ml-4">
             <div className="bg-neutral-300 dark:bg-neutral-700/50 animate-pulse w-24 h-[16px] rounded-md" />
            </div>
           </div>
          ))
      : Array.from({ length: 20 }).map((_, index) => (
         <div key={index} className="group flex justify-between items-center relative bg-gray-200 dark:bg-neutral-700/20 overflow-hidden border-gray-500/25 rounded-lg p-2.5 transition-all duration-200">
          <div className="bg-neutral-300 dark:bg-neutral-700/50 animate-pulse p-1 w-[32px] h-[32px] rounded-md" />
          <div className="relative flex-1 ml-4">
           <div className="bg-neutral-300 dark:bg-neutral-700/50 animate-pulse w-24 h-[16px] rounded-md" />
          </div>
         </div>
        ))}
    </div>
   </div>
  </>
 );
};
