  ...
  <>
   <div className="p-4 w-72 text-left bg-white dark:bg-black shadow-lg rounded-md">
    <div className="flex items-center space-x-3">
     {activity?.assets?.large_image ? <img src={toImgB64(activity.assets.large_image.startsWith("mp:external/") ? `https://media.discordapp.net/external/${activity.assets.large_image.replace("mp:external/", "")}` : `https://cdn.discordapp.com/app-assets/${activity.application_id}/${activity.assets.large_image}.webp`)} alt={activity?.assets?.large_text ? activity.assets.large_text : "Activity"} className="w-10 h-10 rounded-md" /> : <IconUnknown className="w-10 h-10 rounded-md" />}
     <div>
    ...
    <div className="flex items-center space-x-2 mt-3">
     {activity?.assets?.small_image ? <img src={toImgB64(activity.assets.small_image.startsWith("mp:external/") ? `https://media.discordapp.net/external/${activity.assets.small_image.replace("mp:external/", "")}` : `https://cdn.discordapp.com/app-assets/${activity.application_id}/${activity.assets.small_image}.webp`)} alt={activity.assets?.small_text} className="w-5 h-5 rounded-md" /> : <IconUnknown className="w-5 h-5 rounded-md" />}
