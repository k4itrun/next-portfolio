  ...
  <footer className="bg-white/50 dark:bg-black/50 w-full px-6 lg:px-12 py-8 text-zinc-400">
   <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start space-y-4 lg:space-y-0">
    <Link href="/" className="text-2xl font-bold flex items-center space-x-2">
     <span className="bg-color-layout text-xs px-2 py-1 rounded-lg">v{meta.version}</span>
     <span className="hover:text-black dark:hover:text-white transition-colors duration-200">{meta.title}</span>
    </Link>
    <div className="text-center lg:text-right">
     <p className="mb-1">
      {meta.title} &copy; 2020 - {new Date().getFullYear()}, All rights reserved.
     </p>
     <p className="flex items-center justify-center lg:justify-end space-x-2">
      <span>Develop without</span>
      <svg onClick={() => setHeartColor(randomColor())} style={{ cursor: "pointer", fill: heartColor, transition: "fill 0.5s ease" }} xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24">
